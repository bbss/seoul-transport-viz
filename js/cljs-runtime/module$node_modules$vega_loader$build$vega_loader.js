...
"function"===typeof define&&define.amd?define(["exports","vega-util","d3-dsv","topojson-client","vega-format"],factory):(global=global||self,factory(global.vega={},global.vega,global.d3,global.topojson,global.vega))})(this,function(exports,vegaUtil,d3Dsv,topojsonClient,vegaFormat){function load(uri,options){var $jscomp$async$this=this,opt,url;return $jscomp.asyncExecutePromiseGeneratorProgram(function($jscomp$generator$context){if(1==$jscomp$generator$context.nextAddress)return $jscomp$generator$context.yield($jscomp$async$this.sanitize(uri,
...
"\n"+data);return d3Dsv.dsvFormat(format.delimiter).parse(data+"")}function json(data,format){format=format&&format.property?vegaUtil.field(format.property):vegaUtil.identity;var JSCompiler_temp;if(JSCompiler_temp=vegaUtil.isObject(data))JSCompiler_temp=!("function"===typeof Buffer&&vegaUtil.isFunction(Buffer.isBuffer)&&Buffer.isBuffer(data));return JSCompiler_temp?format(data):format(JSON.parse(data))}function topojson(data,format){var object;data=json(data,format);if(format&&format.feature){var method=
topojsonClient.feature;var property=format.feature}else if(format&&format.mesh){method=topojsonClient.mesh;property=format.mesh;var filter=filters[format.filter]}else vegaUtil.error("Missing TopoJSON feature or mesh parameter.");return(object=(object=data.objects[property])?method(data,object,filter):vegaUtil.error("Invalid TopoJSON object: "+property))&&object.features||[object]}function formats(name,reader){return 1<arguments.length?(format$jscomp$0[name]=reader,this):vegaUtil.hasOwnProperty(format$jscomp$0,
name)?format$jscomp$0[name]:null}function parse$jscomp$0(data,types,timeParser,utcParser){if(data.length){var locale=vegaFormat.timeFormatDefaultLocale();timeParser=timeParser||locale.timeParse;utcParser=utcParser||locale.utcParse;locale=data.columns||Object.keys(data[0]);var j,m;"auto"===types&&(types=inferTypes(data,locale));locale=Object.keys(types);var parsers=locale.map(function(field){var type=types[field];if(type&&(type.startsWith("date:")||type.startsWith("utc:"))){field=type.split(/:(.+)?/,
2);type=field[1];if("'"===type[0]&&"'"===type[type.length-1]||'"'===type[0]&&'"'===type[type.length-1])type=type.slice(1,-1);return("utc"===field[0]?utcParser:timeParser)(type)}if(!typeParsers[type])throw Error("Illegal format pattern: "+field+":"+type);return typeParsers[type]});var i=0;var n=data.length;for(m=locale.length;i<n;++i){var datum=data[i];for(j=0;j<m;++j){var field$jscomp$0=locale[j];datum[field$jscomp$0]=parsers[j](datum[field$jscomp$0])}}}}var protocol_re=/^([A-Za-z]+:)?\/\//,allowed_re=
/^(?:(?:(?:f|ht)tps?|mailto|tel|callto|cid|xmpp|file|data):|[^a-z]|[a-z+.\-]+(?:[^a-z+.\-:]|$))/i,whitespace_re=/[\u0000-\u0020\u00A0\u1680\u180E\u2000-\u2029\u205f\u3000]/g,typeParsers={boolean:vegaUtil.toBoolean,integer:vegaUtil.toNumber,number:vegaUtil.toNumber,date:vegaUtil.toDate,string:vegaUtil.toString,unknown:vegaUtil.identity},typeTests=[function(_){return"true"===_||"false"===_||!0===_||!1===_},function(_){return isNumber(_)&&Number.isInteger(+_)},isNumber,function(_){return!Number.isNaN(Date.parse(_))}],
typeList=["boolean","integer","number","date"];dsv.responseType="text";json.responseType="json";var filters={interior:function(a,b){return a!==b},exterior:function(a,b){return a===b}};topojson.responseType="json";var format$jscomp$0={dsv:dsv,csv:delimitedFormat(","),tsv:delimitedFormat("\t"),json:json,topojson:topojson},loader=function(fetch,fs){return function(options){return{options:options||{},sanitize:sanitize,load:load,fileAccess:!!fs,file:fileLoader(fs),http:httpLoader(fetch)}}}("undefined"!==
typeof fetch&&fetch,null);exports.format=format$jscomp$0;exports.formats=formats;exports.inferType=inferType;exports.inferTypes=inferTypes;exports.loader=loader;exports.read=function(data,schema,timeParser,utcParser){schema=schema||{};var reader=formats(schema.type||"json");reader||vegaUtil.error("Unknown data format type: "+schema.type);data=reader(data,schema);schema.parse&&parse$jscomp$0(data,schema.parse,timeParser,utcParser);vegaUtil.hasOwnProperty(data,"columns")&&delete data.columns;return data};
exports.responseType=function(type){return(type=formats(type))&&type.responseType||"text"};exports.typeParsers=typeParsers;Object.defineProperty(exports,"__esModule",{value:!0})})}
