...
value:function(index){if(void 0===index){index=this.data;var offset=0}else offset=this.offsets[index],index=this.data.subarray(offset,offset+this.sizes[index]),offset*=4;return{data:index,offset:offset}}},{key:"setUniforms",value:function(values){for(var key in values)this._setValue(key,values[key]);return this}},{key:"_setValue",value:function(key,value){key=this.layout[key];(0,_utils.assert)(key,"UniformLayoutStd140 illegal argument");var typedArray=this.typedArray[key.type];1===key.size?typedArray[key.offset]=
value:typedArray.set(value,key.offset)}},{key:"_addUniform",value:function(key,uniformType){var typeAndComponents=(0,_attributeUtils.decomposeCompositeGLType)(uniformType);(0,_utils.assert)(typeAndComponents,"UniformBufferLayout illegal argument");uniformType=typeAndComponents.type;typeAndComponents=typeAndComponents.components;var offset=this.size=this._alignTo(this.size,typeAndComponents);this.size+=typeAndComponents;this.layout[key]={type:uniformType,size:typeAndComponents,offset:offset}}},{key:"_alignTo",
