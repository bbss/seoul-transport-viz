shadow$provide.module$node_modules$vega_runtime$build$vega_runtime=function(global$jscomp$0,require,module,exports$jscomp$0){(function(global,factory){"object"===typeof exports$jscomp$0&&"undefined"!==typeof module?factory(exports$jscomp$0,require("module$node_modules$vega_util$build$vega_util"),require("module$node_modules$vega_dataflow$build$vega_dataflow")):"function"===typeof define&&define.amd?define(["exports","vega-util","vega-dataflow"],factory):(global=global||self,factory(global.vega={},
...
transforms,functions,expr){this.dataflow=df;this.transforms=transforms;this.events=df.events.bind(df);this.expr=expr||expressionCodegen;this.signals={};this.scales={};this.nodes={};this.data={};this.fn={};functions&&(this.functions=Object.create(functions),this.functions.context=this)}function Subcontext(ctx){this.dataflow=ctx.dataflow;this.transforms=ctx.transforms;this.events=ctx.events;this.expr=ctx.expr;this.signals=Object.create(ctx.signals);this.scales=Object.create(ctx.scales);this.nodes=Object.create(ctx.nodes);
this.data=Object.create(ctx.data);this.fn=Object.create(ctx.fn);ctx.functions&&(this.functions=Object.create(ctx.functions),this.functions.context=this)}var Skip=vegaUtil.toSet(["rule"]),Swap=vegaUtil.toSet(["group","image","rect"]),expressionCodegen={operator:function(ctx,expr){return expression(ctx,["_"],expr.code)},parameter:function(ctx,expr){return expression(ctx,["datum","_"],expr.code)},event:function(ctx,expr){return expression(ctx,["event"],expr.code)},handler:function(ctx,expr){return expression(ctx,
["_","event"],"var datum\x3devent.item\x26\x26event.item.datum;return "+expr.code+";")},encode:function(ctx,encode){var marktype=encode.marktype;encode=encode.channels;var code="var o\x3ditem,datum\x3do.datum,m\x3d0,$;";for(name in encode){var o="o["+vegaUtil.stringValue(name)+"]";code+="$\x3d"+encode[name].code+";if("+o+"!\x3d\x3d$)"+o+"\x3d$,m\x3d1;"}var name="";Skip[marktype]||(encode.x2&&(encode.x?(Swap[marktype]&&(name+="if(o.x\x3eo.x2)$\x3do.x,o.x\x3do.x2,o.x2\x3d$;"),name+="o.width\x3do.x2-o.x;"):
name+="o.x\x3do.x2-(o.width||0);"),encode.xc&&(name+="o.x\x3do.xc-(o.width||0)/2;"),encode.y2&&(encode.y?(Swap[marktype]&&(name+="if(o.y\x3eo.y2)$\x3do.y,o.y\x3do.y2,o.y2\x3d$;"),name+="o.height\x3do.y2-o.y;"):name+="o.y\x3do.y2-(o.height||0);"),encode.yc&&(name+="o.y\x3do.yc-(o.height||0)/2;"));code+=name;return expression(ctx,["item","_"],code+"return m;")},codegen:{get:function(path){path="["+path.map(vegaUtil.stringValue).join("][")+"]";var get=Function("_","return _"+path+";");get.path=path;
return get},comparator:function(fields,orders){var t;fields=Function("a","b","var u, v; return "+fields.map(function(f,i){var o=orders[i];if(f.path){var u="a"+f.path;f="b"+f.path}else(t=t||{})["f"+i]=f,u="this.f"+i+"(a)",f="this.f"+i+"(b)";i=-o;return"((u \x3d "+u+") \x3c (v \x3d "+f+") || u \x3d\x3d null) \x26\x26 v !\x3d null ? "+i+"\n  : (u \x3e v || v \x3d\x3d null) \x26\x26 u !\x3d null ? "+o+"\n  : ((v \x3d v instanceof Date ? +v : v), (u \x3d u instanceof Date ? +u : u)) !\x3d\x3d u \x26\x26 v \x3d\x3d\x3d v ? "+
...
op);if(spec.data){data={};for(var name in spec.data)data.$jscomp$loop$prop$data$656$692=ctx.data[name]||(ctx.data[name]={}),spec.data[name].forEach(function($jscomp$loop$691){return function(role){return $jscomp$loop$691.$jscomp$loop$prop$data$656$692[role]=op}}(data)),data={$jscomp$loop$prop$data$656$692:data.$jscomp$loop$prop$data$656$692}}},resolve:function(){(this.unresolved||[]).forEach(function(fn){return fn()});delete this.unresolved;return this},operator:function(spec,update){this.add(spec,
