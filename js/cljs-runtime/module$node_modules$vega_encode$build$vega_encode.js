...
"function"===typeof define&&define.amd?define("exports vega-dataflow vega-scale vega-util d3-array d3-interpolate".split(" "),factory):(global=global||self,factory((global.vega=global.vega||{},global.vega.transforms={}),global.vega,global.vega,global.vega,global.d3,global.d3))})(this,function(exports,vegaDataflow,vegaScale,vegaUtil,d3Array,d3Interpolate){function AxisTicks(params){vegaDataflow.Transform.call(this,null,params)}function DataJoin(params){vegaDataflow.Transform.call(this,null,params)}
...
{},params)}function line(sx,sy,tx,ty){return"M"+sx+","+sy+"L"+tx+","+ty}function arc(sx,sy,tx,ty){var dx=tx-sx,dy=ty-sy,rr=Math.sqrt(dx*dx+dy*dy)/2;return"M"+sx+","+sy+"A"+rr+","+rr+" "+180*Math.atan2(dy,dx)/Math.PI+" 0 1 "+tx+","+ty}function curve(sx,sy,tx,ty){var dx=tx-sx,dy=ty-sy,ix=.2*(dx+dy);dx=.2*(dy-dx);return"M"+sx+","+sy+"C"+(sx+ix)+","+(sy+dx)+" "+(tx+dx)+","+(ty-ix)+" "+tx+","+ty}function Pie(params){vegaDataflow.Transform.call(this,null,params)}function includePad(type){return vegaScale.isContinuous(type)&&
...
","+sr+" 0 0,"+((Math.abs(ta-sa)>Math.PI?ta<=sa:ta>sa)?1:0)+" "+sr*tc+","+sr*ts+"L"+tr*tc+","+tr*ts},"diagonal-horizontal":function(sx,sy,tx,ty){var m=(sx+tx)/2;return"M"+sx+","+sy+"C"+m+","+sy+" "+m+","+ty+" "+tx+","+ty},"diagonal-vertical":function(sx,sy,tx,ty){var m=(sy+ty)/2;return"M"+sx+","+sy+"C"+sx+","+m+" "+tx+","+m+" "+tx+","+ty},"diagonal-radial":function(sa,sr,ta,tr){var sc=Math.cos(sa);sa=Math.sin(sa);var tc=Math.cos(ta);ta=Math.sin(ta);var mr=(sr+tr)/2;return"M"+sr*sc+","+sr*sa+"C"+mr*
sc+","+mr*sa+" "+mr*tc+","+mr*ta+" "+tr*tc+","+tr*ta}});LinkPath.Definition={type:"LinkPath",metadata:{modifies:!0},params:[{name:"sourceX",type:"field","default":"source.x"},{name:"sourceY",type:"field","default":"source.y"},{name:"targetX",type:"field","default":"target.x"},{name:"targetY",type:"field","default":"target.y"},{name:"orient",type:"enum","default":"vertical",values:["horizontal","vertical","radial"]},{name:"shape",type:"enum","default":"line",values:["line","arc","curve","diagonal",
"orthogonal"]},{name:"require",type:"signal"},{name:"as",type:"string","default":"path"}]};vegaUtil.inherits(LinkPath,vegaDataflow.Transform).transform=function(_,pulse){var sx=_.sourceX||sourceX,sy=_.sourceY||sourceY,tx=_.targetX||targetX,ty=_.targetY||targetY,as=_.as||"path",shape=_.shape||"line",path=Paths.get(shape+"-"+(_.orient||"vertical"))||Paths.get(shape);path||vegaUtil.error("LinkPath unsupported type: "+_.shape+(_.orient?"-"+_.orient:""));pulse.visit(pulse.SOURCE,function(t){t[as]=path(sx(t),
sy(t),tx(t),ty(t))});return pulse.reflow(_.modified()).modifies(as)};Pie.Definition={type:"Pie",metadata:{modifies:!0},params:[{name:"field",type:"field"},{name:"startAngle",type:"number","default":0},{name:"endAngle",type:"number","default":6.283185307179586},{name:"sort",type:"boolean","default":!1},{name:"as",type:"string",array:!0,length:2,"default":["startAngle","endAngle"]}]};vegaUtil.inherits(Pie,vegaDataflow.Transform).transform=function(_,pulse){var as=_.as||["startAngle","endAngle"],startAngle=
as[0],endAngle=as[1],start=_.startAngle||0,stop=null!=_.endAngle?_.endAngle:2*Math.PI,data=pulse.source,values=data.map(_.field||vegaUtil.one),n=values.length,a$jscomp$0=start;start=(stop-start)/d3Array.sum(values);stop=d3Array.range(n);var i;_.sort&&stop.sort(function(a,b){return values[a]-values[b]});for(i=0;i<n;++i){var v=values[stop[i]];var t=data[stop[i]];t[startAngle]=a$jscomp$0;t[endAngle]=a$jscomp$0+=v*start}this.value=values;return pulse.reflow(_.modified()).modifies(as)};var SKIP=vegaUtil.toSet("set modified clear type scheme schemeExtent schemeCount domain domainMin domainMid domainMax domainRaw domainImplicit nice zero bins range rangeStep round reverse interpolate interpolateGamma".split(" "));
...
d.domain(),scale=JSCompiler_temp[0],JSCompiler_temp=vegaUtil.peek(JSCompiler_temp),domain=null==t.start?scale:t.start,n=null==t.stop?JSCompiler_temp:t.stop,(t=t.step)||vegaUtil.error("Scale bins parameter missing step property."),domain<scale&&(domain=t*Math.ceil(scale/t)),n>JSCompiler_temp&&(n=t*Math.floor(JSCompiler_temp/t)),t=d3Array.range(domain,n+t/2,t));t?d.bins=t:d.bins&&delete d.bins;d.type===vegaScale.BinOrdinal&&(t?_.domain||_.domainRaw||(d.domain(t),df=t.length):d.bins=d.domain());scale=
df;a:{d=key.type;df=_.round||!1;t=_.range;if(null!=_.rangeStep)d!==vegaScale.Band&&d!==vegaScale.Point&&vegaUtil.error("Only band and point scales support rangeStep."),t=[0,_.rangeStep*vegaScale.bandSpace(scale,d===vegaScale.Point?1:(null!=_.paddingInner?_.paddingInner:_.padding)||0,(null!=_.paddingOuter?_.paddingOuter:_.padding)||0)];else if(_.scheme&&(t=_.schemeExtent,vegaUtil.isArray(_.scheme)?JSCompiler_temp=vegaScale.interpolateColors(_.scheme,_.interpolate,_.interpolateGamma):(JSCompiler_temp=
_.scheme.toLowerCase(),(JSCompiler_temp=vegaScale.scheme(JSCompiler_temp))||vegaUtil.error("Unrecognized scheme name: "+_.scheme)),scale=d===vegaScale.Threshold?scale+1:d===vegaScale.BinOrdinal?scale-1:d===vegaScale.Quantile||d===vegaScale.Quantize?+_.schemeCount||5:scale,t=vegaScale.isInterpolating(d)?adjustScheme(JSCompiler_temp,t,_.reverse):vegaUtil.isFunction(JSCompiler_temp)?vegaScale.quantizeInterpolator(adjustScheme(JSCompiler_temp,t),scale):d===vegaScale.Ordinal?JSCompiler_temp:JSCompiler_temp.slice(0,
scale),vegaUtil.isFunction(t)))if(key.interpolator){key.interpolator(t);break a}else vegaUtil.error("Scale type "+d+" does not support interpolating color schemes.");t&&vegaScale.isInterpolating(d)?key.interpolator(vegaScale.interpolateColors(flip(t,_.reverse),_.interpolate,_.interpolateGamma)):(t&&_.interpolate&&key.interpolate?key.interpolate(vegaScale.interpolate(_.interpolate,_.interpolateGamma)):vegaUtil.isFunction(key.round)?key.round(df):vegaUtil.isFunction(key.rangeRound)&&key.interpolate(df?
d3Interpolate.interpolateRound:d3Interpolate.interpolate),t&&key.range(flip(t,_.reverse)))}return pulse.fork(pulse.NO_SOURCE|pulse.NO_FIELDS)};vegaUtil.inherits(SortItems,vegaDataflow.Transform).transform=function(_,pulse){var mod=_.modified("sort")||pulse.changed(pulse.ADD)||pulse.modified(_.sort.fields)||pulse.modified("datum");mod&&pulse.source.sort(vegaDataflow.stableCompare(_.sort));this.modified(mod);return pulse};var DefOutput=["y0","y1"];Stack.Definition={type:"Stack",metadata:{modifies:!0},
params:[{name:"field",type:"field"},{name:"groupby",type:"field",array:!0},{name:"sort",type:"compare"},{name:"offset",type:"enum","default":"zero",values:["zero","center","normalize"]},{name:"as",type:"string",array:!0,length:2,"default":DefOutput}]};vegaUtil.inherits(Stack,vegaDataflow.Transform).transform=function(_,pulse){var as=_.as||DefOutput,y0=as[0],y1=as[1],sort=vegaDataflow.stableCompare(_.sort),field=_.field||vegaUtil.one,stack="center"===_.offset?stackCenter:"normalize"===_.offset?stackNormalize:
