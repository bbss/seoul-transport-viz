...
"`."),valueToIndex.keys?"You can provide one of the following values: ".concat(Array.from(valueToIndex.keys()).join(", "),"."):null].join("\n")),tabMeta=tabMeta?tabMeta.getBoundingClientRect():null));return{tabsMeta:tabsMeta,tabMeta:tabMeta}},updateIndicatorState=(0,_useEventCallback.default)(function(){var _newIndicatorStyle,_getTabsMeta=getTabsMeta(),tabsMeta=_getTabsMeta.tabsMeta;_getTabsMeta=_getTabsMeta.tabMeta;var startValue=0;_getTabsMeta&&tabsMeta&&(startValue=vertical?_getTabsMeta.top-tabsMeta.top+
tabsMeta.scrollTop:_getTabsMeta.left-tabsMeta.left+(isRtl?tabsMeta.scrollLeftNormalized+tabsMeta.clientWidth-tabsMeta.scrollWidth:tabsMeta.scrollLeft));tabsMeta=(_newIndicatorStyle={},(0,_defineProperty2.default)(_newIndicatorStyle,start,startValue),(0,_defineProperty2.default)(_newIndicatorStyle,size,_getTabsMeta?_getTabsMeta[size]:0),_newIndicatorStyle);isNaN(indicatorStyle[start])||isNaN(indicatorStyle[size])?setIndicatorStyle(tabsMeta):(_newIndicatorStyle=Math.abs(indicatorStyle[size]-tabsMeta[size]),
(1<=Math.abs(indicatorStyle[start]-tabsMeta[start])||1<=_newIndicatorStyle)&&setIndicatorStyle(tabsMeta))}),scroll=function(scrollValue){(0,_animate.default)(scrollStart,tabsRef.current,scrollValue)},moveTabsScroll=function(delta){var scrollValue=tabsRef.current[scrollStart];vertical?scrollValue+=delta:(scrollValue+=delta*(isRtl?-1:1),scrollValue*=isRtl&&"reverse"===(0,_scrollLeft.detectScrollType)()?-1:1);scroll(scrollValue)},handleStartScrollClick=function(){moveTabsScroll(-tabsRef.current[clientSize])},
handleEndScrollClick=function(){moveTabsScroll(tabsRef.current[clientSize])},handleScrollbarSizeChange=React.useCallback(function(scrollbarHeight){setScrollerStyle({overflow:null,marginBottom:-scrollbarHeight})},[]),scrollSelectedIntoView=(0,_useEventCallback.default)(function(){var _getTabsMeta2=getTabsMeta(),tabsMeta=_getTabsMeta2.tabsMeta;(_getTabsMeta2=_getTabsMeta2.tabMeta)&&tabsMeta&&(_getTabsMeta2[start]<tabsMeta[start]?scroll(tabsMeta[scrollStart]+(_getTabsMeta2[start]-tabsMeta[start])):_getTabsMeta2[end]>
tabsMeta[end]&&scroll(tabsMeta[scrollStart]+(_getTabsMeta2[end]-tabsMeta[end])))}),updateScrollButtonState=(0,_useEventCallback.default)(function(){if(scrollable&&"off"!==scrollButtons){var _tabsRef$current=tabsRef.current,scrollTop=_tabsRef$current.scrollTop,scrollHeight=_tabsRef$current.scrollHeight,clientHeight=_tabsRef$current.clientHeight,scrollWidth=_tabsRef$current.scrollWidth,clientWidth=_tabsRef$current.clientWidth;vertical?(_tabsRef$current=1<scrollTop,scrollWidth=scrollTop<scrollHeight-
clientHeight-1):(scrollTop=(0,_scrollLeft.getNormalizedScrollLeft)(tabsRef.current,theme.direction),_tabsRef$current=isRtl?scrollTop<scrollWidth-clientWidth-1:1<scrollTop,scrollWidth=isRtl?1<scrollTop:scrollTop<scrollWidth-clientWidth-1);_tabsRef$current===displayScroll.start&&scrollWidth===displayScroll.end||setDisplayScroll({start:_tabsRef$current,end:scrollWidth})}});React.useEffect(function(){var handleResize=(0,_debounce.default)(function(){updateIndicatorState();updateScrollButtonState()}),
win=(0,_ownerWindow.default)(tabsRef.current);win.addEventListener("resize",handleResize);return function(){handleResize.clear();win.removeEventListener("resize",handleResize)}},[updateIndicatorState,updateScrollButtonState]);var handleTabsScroll=React.useCallback((0,_debounce.default)(function(){updateScrollButtonState()}));React.useEffect(function(){return function(){handleTabsScroll.clear()}},[handleTabsScroll]);React.useEffect(function(){setMounted(!0)},[]);React.useEffect(function(){updateIndicatorState();
updateScrollButtonState()});React.useEffect(function(){scrollSelectedIntoView()},[scrollSelectedIntoView,indicatorStyle]);React.useImperativeHandle(action,function(){return{updateIndicator:updateIndicatorState,updateScrollButtons:updateScrollButtonState}},[updateIndicatorState,updateScrollButtonState]);var indicator=React.createElement(_TabIndicator.default,(0,_extends2.default)({className:classes.indicator,orientation:orientation,color:_props$indicatorColor},_props$orientation,{style:(0,_extends2.default)({},
indicatorStyle,_props$orientation.style)})),childIndex=0;action=React.Children.map(childrenProp,function(child){if(!React.isValidElement(child))return null;(0,_reactIs.isFragment)(child)&&console.error("Material-UI: The Tabs component doesn't accept a Fragment as a child.\nConsider providing an array instead.");var childValue=void 0===child.props.value?childIndex:child.props.value;valueToIndex.set(childValue,childIndex);var selected=childValue===value;childIndex+=1;return React.cloneElement(child,
{fullWidth:"fullWidth"===variant,indicator:selected&&!mounted&&indicator,selected:selected,selectionFollowsFocus:selectionFollowsFocus,onChange:onChange,textColor:textColor,value:childValue})});childrenProp=function(){var conditionalElements={};conditionalElements.scrollbarSizeListener=scrollable?React.createElement(_ScrollbarSize.default,{className:classes.scrollable,onChange:handleScrollbarSizeChange}):null;var scrollButtonsActive=displayScroll.start||displayScroll.end;scrollButtonsActive=scrollable&&
("auto"===scrollButtons&&scrollButtonsActive||"desktop"===scrollButtons||"on"===scrollButtons);conditionalElements.scrollButtonStart=scrollButtonsActive?React.createElement(ScrollButtonComponent,(0,_extends2.default)({orientation:orientation,direction:isRtl?"right":"left",onClick:handleStartScrollClick,disabled:!displayScroll.start,className:(0,_clsx.default)(classes.scrollButtons,"on"!==scrollButtons&&classes.scrollButtonsDesktop)},TabScrollButtonProps)):null;conditionalElements.scrollButtonEnd=
scrollButtonsActive?React.createElement(ScrollButtonComponent,(0,_extends2.default)({orientation:orientation,direction:isRtl?"left":"right",onClick:handleEndScrollClick,disabled:!displayScroll.end,className:(0,_clsx.default)(classes.scrollButtons,"on"!==scrollButtons&&classes.scrollButtonsDesktop)},TabScrollButtonProps)):null;return conditionalElements}();return React.createElement(_props$component,(0,_extends2.default)({className:(0,_clsx.default)(classes.root,className,vertical&&classes.vertical),
ref:ref},props),childrenProp.scrollButtonStart,childrenProp.scrollbarSizeListener,React.createElement("div",{className:(0,_clsx.default)(classes.scroller,scrollable?classes.scrollable:classes.fixed),style:_props$textColor,ref:tabsRef,onScroll:handleTabsScroll},React.createElement("div",{"aria-label":ariaLabel,"aria-labelledby":ariaLabelledBy,className:(0,_clsx.default)(classes.flexContainer,vertical&&classes.flexContainerVertical,_props$centered&&!scrollable&&classes.centered),onKeyDown:function(event){var target=
event.target;if("tab"===target.getAttribute("role")){var newFocusTarget=null,previousItemKey="vertical"!==orientation?"ArrowLeft":"ArrowUp",nextItemKey="vertical"!==orientation?"ArrowRight":"ArrowDown";"vertical"!==orientation&&"rtl"===theme.direction&&(previousItemKey="ArrowRight",nextItemKey="ArrowLeft");switch(event.key){case previousItemKey:newFocusTarget=target.previousElementSibling||tabListRef.current.lastChild;break;case nextItemKey:newFocusTarget=target.nextElementSibling||tabListRef.current.firstChild;
break;case "Home":newFocusTarget=tabListRef.current.firstChild;break;case "End":newFocusTarget=tabListRef.current.lastChild}null!==newFocusTarget&&(newFocusTarget.focus(),event.preventDefault())}},ref:tabListRef,role:"tablist"},action),mounted&&indicator),childrenProp.scrollButtonEnd)});_interopRequireDefault.propTypes={action:module.refType,"aria-label":global.default.string,"aria-labelledby":global.default.string,centered:global.default.bool,children:global.default.node,classes:global.default.object,
...
"primary","secondary"]),value:global.default.any,variant:global.default.oneOf(["fullWidth","scrollable","standard"])};require=(0,_withStyles.default)(require,{name:"MuiTabs"})(_interopRequireDefault);exports.default=require}
