...
entries[i],type=_entries$i.type,handler=_entries$i.handler;_entries$i=_entries$i.once;handler(Object.assign({},event,{type:type,stopPropagation:stopPropagation,stopImmediatePropagation:stopImmediatePropagation}));_entries$i&&srcElement.push(entries[i]);if(immediatePropagationStopped)break}for(entries=0;entries<srcElement.length;entries++)handler=srcElement[entries],type=handler.type,handler=handler.handler,this.remove(type,handler)}}},{key:"_normalizeEvent",value:function(event){var rootElement=this.eventManager.element;
return Object.assign({},event,(0,_eventUtils.whichButtons)(event),(0,_eventUtils.getOffsetPosition)(event,rootElement),{handled:!1,rootElement:rootElement})}}]);return EventRegistrar}();exports["default"]=require}
