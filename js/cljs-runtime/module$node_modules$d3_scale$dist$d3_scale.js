...
function(_){if(!arguments.length)return domain.slice();domain=[];index=new Map;for(var $jscomp$iter$314=$jscomp.makeIterator(_),$jscomp$key$value=$jscomp$iter$314.next();!$jscomp$key$value.done;$jscomp$key$value=$jscomp$iter$314.next()){$jscomp$key$value=$jscomp$key$value.value;var key=$jscomp$key$value+"";index.has(key)||index.set(key,domain.push($jscomp$key$value))}return scale};scale.range=function(_){return arguments.length?(range=Array.from(_),scale):range.slice()};scale.unknown=function(_){return arguments.length?
...
scale.copy=function(){return copy$1(scale,sequentialPow()).exponent(scale.exponent())};return initInterpolator.apply(scale,arguments)}function sequentialQuantile(){function scale(x){if(!isNaN(x=+x))return interpolator((d3Array.bisect(domain,x,1)-1)/(domain.length-1))}var domain=[],interpolator=identity;scale.domain=function(_){if(!arguments.length)return domain.slice();domain=[];for(var $jscomp$iter$316=$jscomp.makeIterator(_),$jscomp$key$d=$jscomp$iter$316.next();!$jscomp$key$d.done;$jscomp$key$d=
$jscomp$iter$316.next())$jscomp$key$d=$jscomp$key$d.value,null==$jscomp$key$d||isNaN($jscomp$key$d=+$jscomp$key$d)||domain.push($jscomp$key$d);domain.sort(d3Array.ascending);return scale};scale.interpolator=function(_){return arguments.length?(interpolator=_,scale):interpolator};scale.range=function(){return domain.map(function(d,i){return interpolator(i/(domain.length-1))})};scale.quantiles=function(n){return Array.from({length:n+1},function(_,i){return d3Array.quantile(domain,i/n)})};scale.copy=
