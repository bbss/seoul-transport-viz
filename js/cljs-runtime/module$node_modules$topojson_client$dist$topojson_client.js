...
function(geoms){filter(geoms[0].g,geoms[geoms.length-1].g)&&arcs$jscomp$0.push(geoms[0].i)});return arcs$jscomp$0}function mergeArcs(topology,objects){function geometry(o){switch(o.type){case "GeometryCollection":o.geometries.forEach(geometry);break;case "Polygon":extract(o.arcs);break;case "MultiPolygon":o.arcs.forEach(extract)}}function extract(polygon){polygon.forEach(function(ring){ring.forEach(function(arc){(polygonsByArc[arc=0>arc?~arc:arc]||(polygonsByArc[arc]=[])).push(polygon)})});polygons$jscomp$0.push(polygon)}
function area$jscomp$0(ring){ring=object$jscomp$0(topology,{type:"Polygon",arcs:[ring]}).coordinates[0];for(var i=-1,n=ring.length,a,b=ring[n-1],area=0;++i<n;)a=b,b=ring[i],area+=a[0]*b[1]-a[1]*b[0];return Math.abs(area)}var polygonsByArc={},polygons$jscomp$0=[],groups=[];objects.forEach(geometry);polygons$jscomp$0.forEach(function(polygon$jscomp$0){if(!polygon$jscomp$0._){var group=[],neighbors=[polygon$jscomp$0];polygon$jscomp$0._=1;for(groups.push(group);polygon$jscomp$0=neighbors.pop();)group.push(polygon$jscomp$0),
polygon$jscomp$0.forEach(function(ring){ring.forEach(function(arc){polygonsByArc[0>arc?~arc:arc].forEach(function(polygon){polygon._||(polygon._=1,neighbors.push(polygon))})})})}});polygons$jscomp$0.forEach(function(polygon){delete polygon._});return{type:"MultiPolygon",arcs:groups.map(function(polygons){var arcs=[];polygons.forEach(function(polygon){polygon.forEach(function(ring){ring.forEach(function(arc){2>polygonsByArc[0>arc?~arc:arc].length&&arcs.push(arc)})})});arcs=stitch(topology,arcs);if(1<
(polygons=arcs.length))for(var i=1,k=area$jscomp$0(arcs[0]),ki;i<polygons;++i)(ki=area$jscomp$0(arcs[i]))>k&&(k=arcs[0],arcs[0]=arcs[i],arcs[i]=k,k=ki);return arcs}).filter(function(arcs){return 0<arcs.length})}}function bisect(a,x){for(var lo=0,hi=a.length;lo<hi;){var mid=lo+hi>>>1;a[mid]<x?lo=mid+1:hi=mid}return lo}function untransform(transform){if(null==transform)return identity;var x0,y0,kx=transform.scale[0],ky=transform.scale[1],dx=transform.translate[0],dy=transform.translate[1];return function(input,
...
box=topology.bbox||bbox$jscomp$0(topology);transform=box[0];var y0=box[1],x1=box[2],y1=box[3],n$jscomp$0;transform={scale:[x1-transform?(x1-transform)/(n$jscomp$0-1):1,y1-y0?(y1-y0)/(n$jscomp$0-1):1],translate:[transform,y0]}}var t=untransform(transform),key;y0=topology.objects;x1={};for(key in y0)x1[key]=quantizeGeometry(y0[key]);return{type:"Topology",bbox:box,transform:transform,objects:x1,arcs:topology.arcs.map(function(input){var i=0,j=1,n=input.length,p,output=Array(n);for(output[0]=t(input[0],
0);++i<n;)if((p=t(input[i],i))[0]||p[1])output[j++]=p;1===j&&(output[j++]=[0,0]);output.length=j;return output})}};exports.transform=transform$jscomp$0;exports.untransform=untransform;Object.defineProperty(exports,"__esModule",{value:!0})})}
