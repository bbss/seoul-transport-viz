...
size=attribute.size*(attribute.doublePrecision?2:1);_ref$getData=attribute.byteOffset;var toStartIndices=attribute.startIndices,hasStartIndices=fromStartIndices&&toStartIndices;_ref=getAttributeBufferLength(attribute,_ref.numInstances);var isConstant=attribute.state.constant;if(hasStartIndices||!(fromLength>=_ref)){var toData=isConstant?attribute.value:attribute.getBuffer().getData({srcByteOffset:_ref$getData});if(attribute.settings.normalized&&!isConstant){var getter=getData;getData=function(value,
chunk){return attribute._normalizeConstant(getter(value,chunk))}}hasStartIndices=isConstant?function(i,chunk){return getData(toData,chunk)}:function(i,chunk){return getData(toData.subarray(i,i+size),chunk)};fromLength=buffer.getData({length:fromLength});_ref=new Float32Array(_ref);(0,_arrayUtils.padArray)({source:fromLength,target:_ref,sourceStartIndices:fromStartIndices,targetStartIndices:toStartIndices,size:size,getData:hasStartIndices});buffer.byteLength<_ref.byteLength+_ref$getData&&buffer.reallocate(_ref.byteLength+
_ref$getData);buffer.subData({data:_ref,offset:_ref$getData})}};var _arrayUtils=require("module$node_modules$$deck_DOT_gl$core$dist$es5$utils$array_utils"),DEFAULT_TRANSITION_SETTINGS={interpolation:{duration:0,easing:function(t){return t}},spring:{stiffness:.05,damping:.5}}}
