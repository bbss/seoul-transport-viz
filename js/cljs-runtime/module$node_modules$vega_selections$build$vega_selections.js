...
base[0]||base[1]<lo)return[];base[0]<lo&&(base[0]=lo);base[1]>hi&&(base[1]=hi);return base}};exports.selectionResolve=function(name,op,isMulti){name=(name=this.context.data[name])?name.values.value:[];for(var resolved={},multiRes={},types={},entry,fields,unit$jscomp$0,field$jscomp$0,res,resUnit,n=name.length,i=0,j$jscomp$0,m;i<n;++i){entry=name[i];unit$jscomp$0=entry.unit;fields=entry.fields;entry=entry.values;j$jscomp$0=0;for(m=fields.length;j$jscomp$0<m;++j$jscomp$0)field$jscomp$0=fields[j$jscomp$0],
...
"union";Object.keys(resolved).forEach(function(field){resolved[field]=Object.keys(resolved[field]).map(function(unit){return resolved[field][unit]}).reduce(function(acc,curr){return void 0===acc?curr:ops[types[field]+"_"+op](acc,curr)})});name=Object.keys(multiRes);isMulti&&name.length&&(isMulti={},unit$jscomp$0={},resolved.vlMulti="union"===op?(isMulti.or=name.reduce(function(acc,k){return acc.push.apply(acc,multiRes[k]),acc},[]),isMulti):(unit$jscomp$0.and=name.map(function(k){var $jscomp$compprop53=
{};return $jscomp$compprop53.or=multiRes[k],$jscomp$compprop53}),unit$jscomp$0));return resolved};exports.selectionTest=function(name,datum,op){var data=this.context.data[name];name=data?data.values.value:[];data=data?data["index:unit"]&&data["index:unit"].value:void 0;op="intersect"===op;for(var n=name.length,i=0,entry,miss,count,unit;i<n;++i)if(entry=name[i],data&&op){if(miss=miss||{},count=miss[unit=entry.unit]||0,-1!==count){entry=testPoint(datum,entry);miss[unit]=entry?-1:++count;if(entry&&1===
