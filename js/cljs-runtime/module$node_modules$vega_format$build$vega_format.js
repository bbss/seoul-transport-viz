...
"function"===typeof define&&define.amd?define("exports d3-array d3-format vega-time vega-util d3-time-format".split(" "),factory):(global=global||self,factory(global.vega={},global.d3,global.d3,global.vega,global.vega,global.d3))})(this,function(exports,d3Array,d3Format,vegaTime,vegaUtil,d3TimeFormat){function memoize(method){var cache={};return function(spec){return cache[spec]||(cache[spec]=method(spec))}}function trimZeroes(numberFormat,decimalChar){return function(x){x=numberFormat(x);var dec=
x.indexOf(decimalChar);if(0>dec)return x;a:{var idx=x.lastIndexOf("e");if(!(0<idx)){for(idx=x.length;--idx>dec;){var end=x.charCodeAt(idx);if(48<=end&&57>=end){idx+=1;break a}}idx=void 0}}for(end=idx<x.length?x.slice(idx):"";--idx>dec;)if("0"!==x[idx]){++idx;break}return x.slice(0,idx)+end}}function numberLocale(locale){var format=memoize(locale.format),formatPrefix=locale.formatPrefix;return{format:format,formatPrefix:formatPrefix,formatFloat:function(spec){spec=d3Format.formatSpecifier(spec||",");
