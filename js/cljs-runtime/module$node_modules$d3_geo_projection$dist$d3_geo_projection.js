...
proj.stream=function(stream){var rotate=proj.rotate(),rotateStream=stream_(stream),sphereStream=(proj.rotate([0,0]),stream_(stream));proj.rotate(rotate);rotateStream.sphere=function(){sphereStream.polygonStart();sphereStream.lineStart();outline(sphereStream,root);sphereStream.lineEnd();sphereStream.polygonEnd()};return rotateStream};return proj.angle(null==r?-30:r*degrees)}function outline(stream,node,parent){var point,edges=node.edges,n=edges.length;var edge={type:"MultiPoint",coordinates:node.face};
var notPoles=node.face.filter(function(d){return 90!==abs(d[1])}),b=d3Geo.geoBounds({type:"MultiPoint",coordinates:notPoles});notPoles=!1;var j=-1,dx=b[1][0]-b[0][0];b=180===dx||360===dx?[(b[0][0]+b[1][0])/2,(b[0][1]+b[1][1])/2]:d3Geo.geoCentroid(edge);if(parent)for(;++j<n&&edges[j]!==parent;);++j;for(dx=0;dx<n;++dx)edge=edges[(dx+j)%n],Array.isArray(edge)?(notPoles||(stream.point((point=d3Geo.geoInterpolate(edge[0],b)(1E-6))[0],point[1]),notPoles=!0),stream.point((point=d3Geo.geoInterpolate(edge[1],
b)(1E-6))[0],point[1])):(notPoles=!1,edge!==parent&&outline(stream,edge,node))}function pointEqual$1(a,b){return a&&b&&a[0]===b[0]&&a[1]===b[1]}function sharedEdge(a,b){for(var x,y,n=a.length,found=null,i=0;i<n;++i){x=a[i];for(var j=b.length;0<=--j;)if(y=b[j],x[0]===y[0]&&x[1]===y[1]){if(found)return[found,x];found=x}}}function faceEdges(face){for(var n=face.length,edges=[],a=face[n-1],i=0;i<n;++i)edges.push([a,a=face[i]]);return edges}function hasInverse(node){return node.project.invert||node.children&&
node.children.some(hasInverse)}function collignonK(a,b){a=collignonRaw(a,b);return[a[0]*kx,a[1]]}function dot(a,b){for(var i=0,n=a.length,s=0;i<n;++i)s+=a[i]*b[i];return s}function cross(a,b){return[a[1]*b[2]-a[2]*b[1],a[2]*b[0]-a[0]*b[2],a[0]*b[1]-a[1]*b[0]]}function spherical(cartesian){return[atan2(cartesian[1],cartesian[0])*degrees,asin(max(-1,min(1,cartesian[2])))*degrees]}function cartesian$jscomp$0(coordinates){var lambda=coordinates[0]*radians;coordinates=coordinates[1]*radians;var cosphi=
cos(coordinates);return[cosphi*cos(lambda),cosphi*sin(lambda),sin(coordinates)]}function noop(){}function projectFeatureCollection(o,stream){return{type:"FeatureCollection",features:o.features.map(function(f){return projectFeature(f,stream)})}}function projectFeature(o,stream){return{type:"Feature",id:o.id,properties:o.properties,geometry:projectGeometry(o.geometry,stream)}}function projectGeometryCollection(o$jscomp$0,stream){return{type:"GeometryCollection",geometries:o$jscomp$0.geometries.map(function(o){return projectGeometry(o,
stream)})}}function projectGeometry(o,stream){if(!o)return null;if("GeometryCollection"===o.type)return projectGeometryCollection(o,stream);switch(o.type){case "Point":var sink=sinkPoint;break;case "MultiPoint":sink=sinkPoint;break;case "LineString":sink=sinkLine;break;case "MultiLineString":sink=sinkLine;break;case "Polygon":sink=sinkPolygon;break;case "MultiPolygon":sink=sinkPolygon;break;case "Sphere":sink=sinkPolygon;break;default:return null}d3Geo.geoStream(o,stream(sink));return sink.result()}
function quincuncial(project){function projectQuincuncial(lambda,phi){var t=abs(lambda)<halfPi;phi=project(t?lambda:0<lambda?lambda-pi$jscomp$0:lambda+pi$jscomp$0,phi);lambda=(phi[0]-phi[1])*sqrt1_2;phi=(phi[0]+phi[1])*sqrt1_2;if(t)return[lambda,phi];t=dx*sqrt1_2;var s=0<lambda^0<phi?-1:1;return[s*lambda-sign(phi)*t,s*phi-sign(lambda)*t]}var dx=project(halfPi,0)[0]-project(-halfPi,0)[0];project.invert&&(projectQuincuncial.invert=function(x0,y0){var x=(x0+y0)*sqrt1_2,y=(y0-x0)*sqrt1_2,t=abs(x)<.5*
dx&&abs(y)<.5*dx;if(!t){var d=dx*sqrt1_2,s=0<x^0<y?-1:1;x0=-s*x0+(0<y?1:-1)*d;y0=-s*y0+(0<x?1:-1)*d;x=(-x0-y0)*sqrt1_2;y=(x0-y0)*sqrt1_2}y0=project.invert(x,y);t||(y0[0]+=0<x?pi$jscomp$0:-pi$jscomp$0);return y0});return d3Geo.geoProjection(projectQuincuncial).rotate([-90,-90,45]).clipAngle(179.999)}function peirce(){return quincuncial(guyouRaw).scale(111.48)}function rectangularPolyconicRaw(phi0){function forward(lambda,phi){lambda=sinPhi0?tan(lambda*sinPhi0/2)/sinPhi0:lambda/2;if(!phi)return[2*lambda,
-phi0];lambda=2*atan(lambda*sin(phi));var cotPhi=1/tan(phi);return[sin(lambda)*cotPhi,phi+(1-cos(lambda))*cotPhi-phi0]}var sinPhi0=sin(phi0);forward.invert=function(x,y){if(1E-6>abs(y+=phi0))return[sinPhi0?2*atan(sinPhi0*x/2)/sinPhi0:x,0];var k=x*x+y*y,phi=0,i=10;do{var delta=tan(phi);var secPhi=1/cos(phi),j=k-2*y*phi+phi*phi;phi-=delta=(delta*j+2*(phi-y))/(2+j*secPhi*secPhi+2*(phi-y)*delta)}while(1E-6<abs(delta)&&0<--i);x*=delta=tan(phi);y=tan(abs(y)<abs(phi+1/delta)?.5*asin(x):.5*acos(x)+pi$jscomp$0/
4)/sin(phi);return[sinPhi0?2*atan(sinPhi0*y)/sinPhi0:2*y,phi]};return forward}function robinsonRaw(lambda,phi){var i=min(18,36*abs(phi)/pi$jscomp$0),i0=floor(i);i-=i0;var ax=(k=K$1[i0])[0],ay=k[1],bx=(k=K$1[++i0])[0],by=k[1];i0=(k=K$1[min(19,++i0)])[0];k=k[1];var k;return[lambda*(bx+i*(i0-ax)/2+i*i*(i0-2*bx+ax)/2),(0<phi?halfPi:-halfPi)*(by+i*(k-ay)/2+i*i*(k-2*by+ay)/2)]}function satelliteVerticalRaw(P){function forward(lambda,phi){var cosPhi=cos(phi),k=(P-1)/(P-cosPhi*cos(lambda));return[k*cosPhi*
sin(lambda),k*sin(phi)]}forward.invert=function(x,y){var rho2=x*x+y*y,rho=sqrt(rho2);rho2=(P-sqrt(1-rho2*(P+1)/(P-1)))/((P-1)/rho+rho/(P-1));return[atan2(x*rho2,rho*sqrt(1-rho2*rho2)),rho?asin(y*rho2/rho):0]};return forward}function satelliteRaw(P,omega){function forward(lambda,phi){lambda=vertical(lambda,phi);phi=lambda[1];var A=phi*sinOmega/(P-1)+cosOmega;return[lambda[0]*cosOmega/A,phi/A]}var vertical=satelliteVerticalRaw(P);if(!omega)return vertical;var cosOmega=cos(omega),sinOmega=sin(omega);
forward.invert=function(x,y){var k=(P-1)/(P-1-y*sinOmega);return vertical.invert(k*x,k*y*cosOmega)};return forward}function nonempty(coordinates){return 0<coordinates.length}function normalizePoint(y){return-90===y||90===y?[0,y]:[-180,Math.floor(1E4*y)/1E4]}function clampPoint(p){var x=p[0],y=p[1],clamped=!1;-179.9999>=x?(x=-180,clamped=!0):179.9999<=x&&(x=180,clamped=!0);-89.9999>=y?(y=-90,clamped=!0):89.9999<=y&&(y=90,clamped=!0);return clamped?[x,y]:p}function clampPoints(points){return points.map(clampPoint)}
function extractFragments(rings,polygon,fragments){for(var j=0,m=rings.length;j<m;++j){var ring=rings[j].slice();fragments.push({index:-1,polygon:polygon,ring:ring});for(var i=0,n=ring.length;i<n;++i){var point=ring[i],x=point[0],y=point[1];if(-179.9999>=x||179.9999<=x||-89.9999>=y||89.9999<=y){ring[i]=clampPoint(point);for(point=i+1;point<n;++point){var pointk=ring[point];x=pointk[0];pointk=pointk[1];if(-179.9999<x&&179.9999>x&&-89.9999<pointk&&89.9999>pointk)break}if(point!==i+1){i?(i={index:-1,
polygon:polygon,ring:ring.slice(0,i+1)},i.ring[i.ring.length-1]=normalizePoint(y),fragments[fragments.length-1]=i):fragments.pop();if(point>=n)break;fragments.push({index:-1,polygon:polygon,ring:ring=ring.slice(point-1)});ring[0]=normalizePoint(ring[0][1]);i=-1;n=ring.length}}}}}function stitchFragments(fragments){var i,n=fragments.length,fragmentByStart={},fragmentByEnd={};for(i=0;i<n;++i){var fragment=fragments[i];var start=fragment.ring[0];var end=fragment.ring[fragment.ring.length-1];start[0]===
end[0]&&start[1]===end[1]?(fragment.polygon.push(fragment.ring),fragments[i]=null):(fragment.index=i,fragmentByStart[start]=fragmentByEnd[end]=fragment)}for(i=0;i<n;++i)if(fragment=fragments[i]){start=fragment.ring[0];end=fragment.ring[fragment.ring.length-1];var startFragment=fragmentByEnd[start];var endFragment=fragmentByStart[end];delete fragmentByStart[start];delete fragmentByEnd[end];start[0]===end[0]&&start[1]===end[1]?fragment.polygon.push(fragment.ring):startFragment?(delete fragmentByEnd[start],
...
max(parallels,1E-6);poleline=min(poleline,halfPi);parallels=min(parallels,pi$jscomp$0-1E-6);inflation=max(inflation,0);inflation=min(inflation,99.999999);ratio=max(ratio,1E-6);var vratio=ratio/100;inflation=acos((inflation/100+1)*cos(phi1))/phi1;phi1=sin(poleline)/sin(inflation*halfPi);ratio=parallels/pi$jscomp$0;parallels=sqrt(vratio*sin(poleline/2)/sin(parallels/2));poleline=parallels/sqrt(ratio*phi1*inflation);parallels=1/(parallels*sqrt(ratio*phi1*inflation));return wagnerFormula(poleline,parallels,
phi1,inflation,ratio)}function wagner(){var poleline=65*radians,parallels=60*radians,inflation=20,ratio=200,mutate=d3Geo.geoProjectionMutator(wagnerRaw),projection=mutate(poleline,parallels,inflation,ratio);projection.poleline=function(_){return arguments.length?mutate(poleline=+_*radians,parallels,inflation,ratio):poleline*degrees};projection.parallels=function(_){return arguments.length?mutate(poleline,parallels=+_*radians,inflation,ratio):parallels*degrees};projection.inflation=function(_){return arguments.length?
...
A+1)-A):0,halfPi*sin(2*atan(y))]};vanDerGrinten4Raw.invert=function(x,y){if(!x||!y)return[x,y];y/=pi$jscomp$0;var x1=sign(x)*x/halfPi,D=(x1*x1-1+4*y*y)/abs(x1),D2=D*D;y*=2;var i=50;do{var B2=y*y;var delta=(8*y-B2*(B2+2)-5)/(2*B2*(y-1));var C_=(3*y-B2*y-10)/(2*B2*y),C2=delta*delta,BC=y*delta,B_C=y+delta,B_C2=B_C*B_C,B_3C=y+3*delta,F_=-2*B_C*(4*BC*C2+(1-4*B2+3*B2*B2)*(1+C_)+C2*(-6+14*B2-D2+(-8+8*B2-2*D2)*C_)+BC*(-8+12*B2+(-10+10*B2-D2)*C_));B2=sqrt(B_C2*(B2+C2*D2-1)+(1-B2)*(B2*(B_3C*B_3C+4*C2)+C2*(12*
BC+4*C2)));y-=delta=(D*(B_C2+C2-1)+2*B2-x1*(4*B_C2+D2))/(D*(2*delta*C_+2*B_C*(1+C_))+F_/B2-8*B_C*(D*(-1+C2+B_C2)+2*B2)*(1+C_)/(D2+4*B_C2))}while(1E-6<delta&&0<--i);return[sign(x)*(sqrt(D*D+4)+D)*pi$jscomp$0/4,halfPi*y]};var A$jscomp$0=4*pi$jscomp$0+3*sqrt(3),B$jscomp$0=2*sqrt(2*pi$jscomp$0*sqrt(3)/A$jscomp$0),wagner4Raw=mollweideBromleyRaw(B$jscomp$0*sqrt(3)/pi$jscomp$0,B$jscomp$0,A$jscomp$0/6);wagner6Raw.invert=function(x,y){return[x/sqrt(1-3*y*y/(pi$jscomp$0*pi$jscomp$0)),y]};wiechelRaw.invert=
function(x,y){var w=(x*x+y*y)/-2,k=sqrt(-w*(2+w)),b=y*w+x*k;x=x*w-y*k;y=sqrt(x*x+b*b);return[atan2(k*b,y*(1+w)),y?-asin(k*x/y):0]};winkel3Raw.invert=function(x,y){var lambda=x,phi=y,i=25;do{var cosphi=cos(phi),sinphi=sin(phi),sin_2phi=sin(2*phi),sin2phi=sinphi*sinphi,cos2phi=cosphi*cosphi,sinlambda=sin(lambda),coslambda_2=cos(lambda/2),sinlambda_2=sin(lambda/2),sin2lambda_2=sinlambda_2*sinlambda_2,C=1-cos2phi*coslambda_2*coslambda_2,E=C?acos(cosphi*coslambda_2)*sqrt(F=1/C):F=0,F;C=.5*(2*E*cosphi*
sinlambda_2+lambda/halfPi)-x;var fy=.5*(E*sinphi+phi)-y,dxdlambda=.5*F*(cos2phi*sin2lambda_2+E*cosphi*coslambda_2*sin2phi)+.5/halfPi,dxdphi=F*(sinlambda*sin_2phi/4-E*sinphi*sinlambda_2);sinphi=.125*F*(sin_2phi*sinlambda_2-E*sinphi*cos2phi*sinlambda);sin2phi=.5*F*(sin2phi*coslambda_2+E*sin2lambda_2*cosphi)+.5;cosphi=dxdphi*sinphi-sin2phi*dxdlambda;dxdphi=(fy*dxdphi-C*sin2phi)/cosphi;C=(C*sinphi-fy*dxdlambda)/cosphi;lambda-=dxdphi;phi-=C}while((1E-6<abs(dxdphi)||1E-6<abs(C))&&0<--i);return[lambda,phi]};
exports.geoNaturalEarth=d3Geo.geoNaturalEarth1;exports.geoNaturalEarthRaw=d3Geo.geoNaturalEarth1Raw;exports.geoAiry=function(){var beta=halfPi,m=d3Geo.geoProjectionMutator(airyRaw),p=m(beta);p.radius=function(_){return arguments.length?m(beta=_*radians):beta*degrees};return p.scale(179.976).clipAngle(147)};exports.geoAiryRaw=airyRaw;exports.geoAitoff=function(){return d3Geo.geoProjection(aitoffRaw).scale(152.63)};exports.geoAitoffRaw=aitoffRaw;exports.geoArmadillo=function(){var phi0=20*radians,sPhi0=
0<=phi0?1:-1,tanPhi0=tan(sPhi0*phi0),m=d3Geo.geoProjectionMutator(armadilloRaw),p=m(phi0),stream_=p.stream;p.parallel=function(_){if(!arguments.length)return phi0*degrees;tanPhi0=tan((sPhi0=0<=(phi0=_*radians)?1:-1)*phi0);return m(phi0)};p.stream=function(stream){var rotate=p.rotate(),rotateStream=stream_(stream),sphereStream=(p.rotate([0,0]),stream_(stream)),precision=p.precision();p.rotate(rotate);rotateStream.sphere=function(){sphereStream.polygonStart();sphereStream.lineStart();for(var lambda=
-180*sPhi0;180>sPhi0*lambda;lambda+=90*sPhi0)sphereStream.point(lambda,90*sPhi0);if(phi0)for(;-180<=sPhi0*(lambda-=3*sPhi0*precision);)sphereStream.point(lambda,sPhi0*-atan2(cos(lambda*radians/2),tanPhi0)*degrees);sphereStream.lineEnd();sphereStream.polygonEnd()};return rotateStream};return p.scale(218.695).center([0,28.0974])};exports.geoArmadilloRaw=armadilloRaw;exports.geoAugust=function(){return d3Geo.geoProjection(augustRaw).scale(66.1603)};exports.geoAugustRaw=augustRaw;exports.geoBaker=function(){return d3Geo.geoProjection(bakerRaw).scale(112.314)};
exports.geoBakerRaw=bakerRaw;exports.geoBerghaus=function(){var lobes=5,m=d3Geo.geoProjectionMutator(berghausRaw),p=m(lobes),projectionStream=p.stream,cr=-cos(.01*radians),sr=sin(.01*radians);p.lobes=function(_){return arguments.length?m(lobes=+_):lobes};p.stream=function(stream){var rotate=p.rotate(),rotateStream=projectionStream(stream),sphereStream=(p.rotate([0,0]),projectionStream(stream));p.rotate(rotate);rotateStream.sphere=function(){sphereStream.polygonStart();sphereStream.lineStart();for(var i=
0,delta=360/lobes,delta0=2*pi$jscomp$0/lobes,phi=90-180/lobes,phi0=halfPi;i<lobes;++i,phi-=delta,phi0-=delta0)sphereStream.point(atan2(sr*cos(phi0),cr)*degrees,asin(sr*sin(phi0))*degrees),-90>phi?(sphereStream.point(-90,-180-phi-.01),sphereStream.point(-90,-180-phi+.01)):(sphereStream.point(90,phi+.01),sphereStream.point(90,phi-.01));sphereStream.lineEnd();sphereStream.polygonEnd()};return rotateStream};return p.scale(87.8076).center([0,17.1875]).clipAngle(179.999)};exports.geoBerghausRaw=berghausRaw;
exports.geoBertin1953=function(){return d3Geo.geoProjection(bertin1953Raw()).rotate([-16.5,-42]).scale(176.57).center([7.93,.09])};exports.geoBertin1953Raw=bertin1953Raw;exports.geoBoggs=function(){return d3Geo.geoProjection(boggsRaw).scale(160.857)};exports.geoBoggsRaw=boggsRaw;exports.geoBonne=function(){return parallel1(bonneRaw).scale(123.082).center([0,26.1441]).parallel(45)};exports.geoBonneRaw=bonneRaw;exports.geoBottomley=function(){var sinPsi=.5,m=d3Geo.geoProjectionMutator(bottomleyRaw),
...
function(_){return arguments.length?m(H=+_):H};p.stream=function(stream){var rotate=p.rotate(),rotateStream=stream_(stream),sphereStream=(p.rotate([0,0]),stream_(stream));p.rotate(rotate);rotateStream.sphere=function(){d3Geo.geoStream(sphere$jscomp$0(180/H),sphereStream)};return rotateStream};return p.scale(239.75)};exports.geoHealpixRaw=healpixRaw;exports.geoHill=function(){var K=1,m=d3Geo.geoProjectionMutator(hillRaw),p=m(K);p.ratio=function(_){return arguments.length?m(K=+_):K};return p.scale(167.774).center([0,
18.67])};exports.geoHillRaw=hillRaw;exports.geoHomolosine=function(){return d3Geo.geoProjection(homolosineRaw).scale(152.63)};exports.geoHomolosineRaw=homolosineRaw;exports.geoHufnagel=function(){var a=1,b=0,psiMax=45*radians,ratio=2,mutate=d3Geo.geoProjectionMutator(hufnagelRaw),projection=mutate(a,b,psiMax,ratio);projection.a=function(_){return arguments.length?mutate(a=+_,b,psiMax,ratio):a};projection.b=function(_){return arguments.length?mutate(a,b=+_,psiMax,ratio):b};projection.psiMax=function(_){return arguments.length?
mutate(a,b,psiMax=+_*radians,ratio):psiMax*degrees};projection.ratio=function(_){return arguments.length?mutate(a,b,psiMax,ratio=+_):ratio};return projection.scale(180.739)};exports.geoHufnagelRaw=hufnagelRaw;exports.geoHyperelliptical=function(){var alpha=0,k=2.5,gamma=1.183136,m=d3Geo.geoProjectionMutator(hyperellipticalRaw),p=m(alpha,k,gamma);p.alpha=function(_){return arguments.length?m(alpha=+_,k,gamma):alpha};p.k=function(_){return arguments.length?m(alpha,k=+_,gamma):k};p.gamma=function(_){return arguments.length?
m(alpha,k,gamma=+_):gamma};return p.scale(152.63)};exports.geoHyperellipticalRaw=hyperellipticalRaw;exports.geoInterrupt=interrupt;exports.geoInterruptedBoggs=function(){return interrupt(boggsRaw,lobes).scale(160.857)};exports.geoInterruptedHomolosine=function(){return interrupt(homolosineRaw,lobes$1).scale(152.63)};exports.geoInterruptedMollweide=function(){return interrupt(mollweideRaw,lobes$2).scale(169.529)};exports.geoInterruptedMollweideHemispheres=function(){return interrupt(mollweideRaw,lobes$3).scale(169.529).rotate([20,
0])};exports.geoInterruptedSinuMollweide=function(){return interrupt(sinuMollweideRaw,lobes$4,solve2d).rotate([-20,-55]).scale(164.263).center([0,-5.4036])};exports.geoInterruptedSinusoidal=function(){return interrupt(sinusoidalRaw,lobes$5).scale(152.63).rotate([-20,0])};exports.geoKavrayskiy7=function(){return d3Geo.geoProjection(kavrayskiy7Raw).scale(158.837)};exports.geoKavrayskiy7Raw=kavrayskiy7Raw;exports.geoLagrange=function(){var n=.5,m=d3Geo.geoProjectionMutator(lagrangeRaw),p=m(n);p.spacing=
function(_){return arguments.length?m(n=+_):n};return p.scale(124.75)};exports.geoLagrangeRaw=lagrangeRaw;exports.geoLarrivee=function(){return d3Geo.geoProjection(larriveeRaw).scale(97.2672)};exports.geoLarriveeRaw=larriveeRaw;exports.geoLaskowski=function(){return d3Geo.geoProjection(laskowskiRaw).scale(139.98)};exports.geoLaskowskiRaw=laskowskiRaw;exports.geoLittrow=function(){return d3Geo.geoProjection(littrowRaw).scale(144.049).clipAngle(89.999)};exports.geoLittrowRaw=littrowRaw;exports.geoLoximuthal=
...
exports.geoModifiedStereographicGs48=function(){return modifiedStereographic(gs48,[95,-38]).scale(1E3).clipAngle(55).center([-96.5563,38.8675])};exports.geoModifiedStereographicGs50=function(){return modifiedStereographic(gs50,[120,-45]).scale(359.513).clipAngle(55).center([-117.474,53.0628])};exports.geoModifiedStereographicMiller=function(){return modifiedStereographic(miller$1,[-20,-18]).scale(209.091).center([20,16.7214]).clipAngle(82)};exports.geoModifiedStereographicLee=function(){return modifiedStereographic(lee,
[165,10]).scale(250).clipAngle(130).center([-165,-10])};exports.geoMollweide=function(){return d3Geo.geoProjection(mollweideRaw).scale(169.529)};exports.geoMollweideRaw=mollweideRaw;exports.geoMtFlatPolarParabolic=function(){return d3Geo.geoProjection(mtFlatPolarParabolicRaw).scale(164.859)};exports.geoMtFlatPolarParabolicRaw=mtFlatPolarParabolicRaw;exports.geoMtFlatPolarQuartic=function(){return d3Geo.geoProjection(mtFlatPolarQuarticRaw).scale(188.209)};exports.geoMtFlatPolarQuarticRaw=mtFlatPolarQuarticRaw;
exports.geoMtFlatPolarSinusoidal=function(){return d3Geo.geoProjection(mtFlatPolarSinusoidalRaw).scale(166.518)};exports.geoMtFlatPolarSinusoidalRaw=mtFlatPolarSinusoidalRaw;exports.geoNaturalEarth2=function(){return d3Geo.geoProjection(naturalEarth2Raw).scale(175.295)};exports.geoNaturalEarth2Raw=naturalEarth2Raw;exports.geoNellHammer=function(){return d3Geo.geoProjection(nellHammerRaw).scale(152.63)};exports.geoNellHammerRaw=nellHammerRaw;exports.geoInterruptedQuarticAuthalic=function(){return interrupt(hammerRaw(Infinity),
lobes$6).rotate([20,0]).scale(152.63)};exports.geoNicolosi=function(){return d3Geo.geoProjection(nicolosiRaw).scale(127.267)};exports.geoNicolosiRaw=nicolosiRaw;exports.geoPatterson=function(){return d3Geo.geoProjection(pattersonRaw).scale(139.319)};exports.geoPattersonRaw=pattersonRaw;exports.geoPolyconic=function(){return d3Geo.geoProjection(polyconicRaw).scale(103.74)};exports.geoPolyconicRaw=polyconicRaw;exports.geoPolyhedral=polyhedral;exports.geoPolyhedralButterfly=function(faceProjection){faceProjection=
faceProjection||function(face){face=d3Geo.geoCentroid({type:"MultiPoint",coordinates:face});return d3Geo.geoGnomonic().scale(1).translate([0,0]).rotate([-face[0],-face[1]])};var faces=octahedron$1.map(function(face){return{face:face,project:faceProjection(face)}});[-1,0,0,1,0,1,4,5].forEach(function(d,i){(d=faces[d])&&(d.children||(d.children=[])).push(faces[i])});return polyhedral(faces[0],function(lambda,phi){return faces[lambda<-pi$jscomp$0/2?0>phi?6:4:0>lambda?0>phi?2:0:lambda<pi$jscomp$0/2?0>
phi?3:1:0>phi?7:5]}).angle(-30).scale(101.858).center([0,45])};exports.geoPolyhedralCollignon=function(faceProjection){faceProjection=faceProjection||function(face){face=d3Geo.geoCentroid({type:"MultiPoint",coordinates:face});return d3Geo.geoProjection(collignonK).translate([0,0]).scale(1).rotate(0<face[1]?[-face[0],0]:[180-face[0],180])};var faces=octahedron$1.map(function(face){return{face:face,project:faceProjection(face)}});[-1,0,0,1,0,1,4,5].forEach(function(d,i){(d=faces[d])&&(d.children||(d.children=
[])).push(faces[i])});return polyhedral(faces[0],function(lambda,phi){return faces[lambda<-pi$jscomp$0/2?0>phi?6:4:0>lambda?0>phi?2:0:lambda<pi$jscomp$0/2?0>phi?3:1:0>phi?7:5]}).angle(-30).scale(121.906).center([0,48.5904])};exports.geoPolyhedralWaterman=function(faceProjection){faceProjection=faceProjection||function(face){face=6===face.length?d3Geo.geoCentroid({type:"MultiPoint",coordinates:face}):face[0];return d3Geo.geoGnomonic().scale(1).translate([0,0]).rotate([-face[0],-face[1]])};var w5=octahedron$1.map(function(face){face=
face.map(cartesian$jscomp$0);for(var n=face.length,a=face[n-1],b,hexagon=[],i=0;i<n;++i)b=face[i],hexagon.push(spherical([.9486832980505138*a[0]+.31622776601683794*b[0],.9486832980505138*a[1]+.31622776601683794*b[1],.9486832980505138*a[2]+.31622776601683794*b[2]]),spherical([.9486832980505138*b[0]+.31622776601683794*a[0],.9486832980505138*b[1]+.31622776601683794*a[1],.9486832980505138*b[2]+.31622776601683794*a[2]])),a=b;return hexagon}),cornerNormals=[],parents=[-1,0,0,1,0,1,4,5];w5.forEach(function(hexagon,
j){for(var face=octahedron$1[j],n=face.length,normals=cornerNormals[j]=[],i=0;i<n;++i)w5.push([face[i],hexagon[(2*i+2)%(2*n)],hexagon[(2*i+1)%(2*n)]]),parents.push(j),normals.push(cross(cartesian$jscomp$0(hexagon[(2*i+2)%(2*n)]),cartesian$jscomp$0(hexagon[(2*i+1)%(2*n)])))});var faces=w5.map(function(face){return{project:faceProjection(face),face:face}});parents.forEach(function(d,i){(d=faces[d])&&(d.children||(d.children=[])).push(faces[i])});return polyhedral(faces[0],function(lambda,phi){var cosphi=
cos(phi);cosphi=[cosphi*cos(lambda),cosphi*sin(lambda),sin(phi)];lambda=lambda<-pi$jscomp$0/2?0>phi?6:4:0>lambda?0>phi?2:0:lambda<pi$jscomp$0/2?0>phi?3:1:0>phi?7:5;phi=cornerNormals[lambda];return faces[0>dot(phi[0],cosphi)?8+3*lambda:0>dot(phi[1],cosphi)?3*lambda+9:0>dot(phi[2],cosphi)?3*lambda+10:lambda]}).angle(-30).scale(110.625).center([0,45])};exports.geoProject=function(object,projection){projection=projection.stream;if(!projection)throw Error("invalid projection");switch(object&&object.type){case "Feature":var project=
projectFeature;break;case "FeatureCollection":project=projectFeatureCollection;break;default:project=projectGeometry}return project(object,projection)};exports.geoGringortenQuincuncial=function(){return quincuncial(gringortenRaw).scale(176.423)};exports.geoPeirceQuincuncial=peirce;exports.geoPierceQuincuncial=peirce;exports.geoQuantize=function(input$jscomp$0,digits){function quantizePoint(input){var n=input.length,i=2,output=Array(n);output[0]=+input[0].toFixed(digits);for(output[1]=+input[1].toFixed(digits);i<
n;)output[i]=input[i],++i;return output}function quantizePointsNoDuplicates(input){for(var point0=quantizePoint(input[0]),output=[point0],i=1;i<input.length;i++){var point=quantizePoint(input[i]);if(2<point.length||point[0]!=point0[0]||point[1]!=point0[1])output.push(point),point0=point}1===output.length&&1<input.length&&output.push(quantizePoint(input[input.length-1]));return output}function quantizePolygon(input){return input.map(quantizePointsNoDuplicates)}function quantizeGeometry(input){if(null==
input)return input;switch(input.type){case "GeometryCollection":var output={type:"GeometryCollection",geometries:input.geometries.map(quantizeGeometry)};break;case "Point":output={type:"Point",coordinates:quantizePoint(input.coordinates)};break;case "MultiPoint":output={type:input.type,coordinates:input.coordinates.map(quantizePoint)};break;case "LineString":output={type:input.type,coordinates:quantizePointsNoDuplicates(input.coordinates)};break;case "MultiLineString":case "Polygon":output={type:input.type,
coordinates:quantizePolygon(input.coordinates)};break;case "MultiPolygon":output={type:"MultiPolygon",coordinates:input.coordinates.map(quantizePolygon)};break;default:return input}null!=input.bbox&&(output.bbox=input.bbox);return output}function quantizeFeature(input){var output={type:"Feature",properties:input.properties,geometry:quantizeGeometry(input.geometry)};null!=input.id&&(output.id=input.id);null!=input.bbox&&(output.bbox=input.bbox);return output}if(!(0<=(digits=+digits)&&20>=digits))throw Error("invalid digits");
if(null!=input$jscomp$0)switch(input$jscomp$0.type){case "Feature":return quantizeFeature(input$jscomp$0);case "FeatureCollection":var output$jscomp$0={type:"FeatureCollection",features:input$jscomp$0.features.map(quantizeFeature)};null!=input$jscomp$0.bbox&&(output$jscomp$0.bbox=input$jscomp$0.bbox);return output$jscomp$0;default:return quantizeGeometry(input$jscomp$0)}return input$jscomp$0};exports.geoQuincuncial=quincuncial;exports.geoRectangularPolyconic=function(){return parallel1(rectangularPolyconicRaw).scale(131.215)};
...
distance)*degrees-1E-6)};exports.geoSatelliteRaw=satelliteRaw;exports.geoSinuMollweide=function(){return d3Geo.geoProjection(sinuMollweideRaw).rotate([-20,-55]).scale(164.263).center([0,-5.4036])};exports.geoSinuMollweideRaw=sinuMollweideRaw;exports.geoSinusoidal=function(){return d3Geo.geoProjection(sinusoidalRaw).scale(152.63)};exports.geoSinusoidalRaw=sinusoidalRaw;exports.geoStitch=function(input){if(null==input)return input;switch(input.type){case "Feature":return stitchFeature(input);case "FeatureCollection":var output=
{type:"FeatureCollection",features:input.features.map(stitchFeature)};null!=input.bbox&&(output.bbox=input.bbox);return output;default:return stitchGeometry(input)}};exports.geoTimes=function(){return d3Geo.geoProjection(timesRaw).scale(146.153)};exports.geoTimesRaw=timesRaw;exports.geoTwoPointAzimuthal=twoPointAzimuthal;exports.geoTwoPointAzimuthalRaw=twoPointAzimuthalRaw;exports.geoTwoPointAzimuthalUsa=function(){return twoPointAzimuthal([-158,21.5],[-77,39]).clipAngle(60).scale(400)};exports.geoTwoPointEquidistant=
twoPointEquidistant;exports.geoTwoPointEquidistantRaw=twoPointEquidistantRaw;exports.geoTwoPointEquidistantUsa=function(){return twoPointEquidistant([-158,21.5],[-77,39]).clipAngle(130).scale(122.571)};exports.geoVanDerGrinten=function(){return d3Geo.geoProjection(vanDerGrintenRaw).scale(79.4183)};exports.geoVanDerGrintenRaw=vanDerGrintenRaw;exports.geoVanDerGrinten2=function(){return d3Geo.geoProjection(vanDerGrinten2Raw).scale(79.4183)};exports.geoVanDerGrinten2Raw=vanDerGrinten2Raw;exports.geoVanDerGrinten3=
